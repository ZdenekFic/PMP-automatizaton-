...
exports.Overviews = class Overviews {
  constructor(page, menuText, testTitle, expectedURL) {
    ...
    //main tabs
    this.mainMenuTabsButtons = page
      .getByRole("navigation")
      .locator("span")
      .filter({ hasText: menuText });
    this.reportsHeader = page.locator(
      "//span[@class='definition-padding-right']"
    );
    this.mainMenuTabsHeader = page
      .getByRole("main")
    ...
    this.definitionsDomainDataModelsMenuButtonPath = page
      ...
      .locator("span")
      .filter({ hasText: menuText });
    this.definitionsDGLMenuButtonPath = page
      .getByRole("navigation")
      .locator("span")
      .filter({ hasText: menuText });
    this.definitionsPBBsMenuButtonPath = page
      .locator("span")
      .filter({ hasText: menuText });
    this.definitionsPBBsQueriesMenuButtonPath = page.getByText(menuText);
    this.definitionsGDMMenuButtonPath = page
      .locator("span")
      .filter({ hasText: menuText });
    this.definitionsContentBricksMenuButtonPath = page
      .locator("span")
      .filter({ hasText: menuText })
      .first();
    this.definitionsSubContentBricksMenuButtonPath = page
      .locator("span")
      .filter({ hasText: menuText });
    this.definitionsTitleHeader = page
      .getByRole("main")
      .getByText(testTitle, { exact: true });

    //administration tabs
    this.mainAdministrationTab = page.getByRole("button", {
      name: "Administration",
      exact: true,
    });
    this.administrationFormattingMenuButtonPath = page
      .getByRole("navigation")
      .locator("span")
      .filter({ hasText: menuText });
    this.administrationListsMenuButtonPath = page
      .getByRole("navigation")
      .locator("span")
      .filter({ hasText: menuText });
    this.administrationQualificationsMenuButtonPath = page.getByText(menuText);
    this.administrationTagsAndSearchIdsMenuButtonPath = page
      .locator("span")
      .filter({ hasText: menuText });
    this.administrationTranslationsMenuButtonPath = page
      .locator("span")
      .filter({ hasText: menuText });
    this.administrationUnitsMenuButtonPath = page
      .getByRole("navigation")
      .locator("span")
      .filter({ hasText: menuText });
    this.administrationUsageTypesMenuButtonPath = page.getByText(menuText);
    this.administrationsPath = page
      .locator("span")
      .filter({ hasText: menuText });
    this.expectTranslation = page.getByRole("tab", { name: testTitle });
    this.expectUnited = page
      .getByRole("main")
      .getByText(testTitle, { exact: true });

    //User administration tabs
    this.mainUserAdministrationTabs = page.getByRole("button", {
      name: "User Administration",
    });
    this.userAdministrationOrganigramMenuButtonPath = page
      .getByRole("navigation")
      .locator("span")
      .filter({ hasText: menuText });
    this.userAdministrationTabsPathUnited = page
      .getByRole("navigation")
      .locator("span")
      .filter({ hasText: menuText });
    this.userAdministrationTabsTitleHeader = page
      .getByRole("main")
      .getByText(testTitle, { exact: true });
  }
