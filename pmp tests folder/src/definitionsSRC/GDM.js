...
exports.GDM = class GDM {
    constructor(page, gdmName) {
      ...
      this.filter = page.locator("//div[@class='v-expansion-panel-header__icon']//i[@class='v-icon notranslate mdi mdi-chevron-down theme--light']");
      this.myRecentItemsCheckbox = page.locator("//div[@class='v-input--selection-controls__ripple primary--text']");
      ...
      //tag1
      this.tag1 = page.locator("//div[@class='v-dialog v-dialog--active v-dialog--persistent v-dialog--scrollable']//tbody/tr[1]/td[1]/div[1]/i[1]");
      //tag2
      this.tag2 = page.locator("//div[@class='v-dialog v-dialog--active v-dialog--persistent v-dialog--scrollable']//tbody/tr[2]/td[1]/div[1]/i[1]");
      //tag2 text
      this.tag2text = page.locator("//div[@class='v-dialog v-dialog--active v-dialog--persistent v-dialog--scrollable']//div[@class='v-card v-sheet theme--light']//tbody/tr[2]/td[3]");
      //tag3
      this.tag3 = page.locator("//div[@class='v-dialog v-dialog--active v-dialog--persistent v-dialog--scrollable']//tbody/tr[5]/td[1]/div[1]/i[1]");
      //Confirm Tags
      this.tagConfirm = page.locator("//span[normalize-space()='Update Tags']");
      //tagDiv
      this.tagDiv = page.locator("//div[@class='v-input v-input--is-label-active v-input--is-dirty theme--light v-text-field v-text-field--is-booted v-select v-select--chips v-select--is-multi v-autocomplete']");

      //OWNER
      this.ownerInput = page.locator("//button[@class='add-reference-textfield-append v-btn v-btn--flat v-btn--icon v-btn--round v-btn--tile theme--light elevation-2 v-size--default error--text']");
      this.owner1 = page.locator("//div[@class='v-dialog v-dialog--active v-dialog--persistent v-dialog--scrollable']//tbody/tr[1]/td[1]");
      this.ownerConfirm = page.locator("//span[normalize-space()='Update Owner']")

      //SAVE part
      this.saveButton = page.locator('.v-toolbar__content > button:nth-child(4)');
      this.dataModelTabCheck = page.locator("//div[normalize-space()='Data model']");
      ...
      this.searchedObject =page.locator(`//div[@class='v-list-item__title'][normalize-space()='${gdmName}']`);
      this.searchedArea = page.locator("//div[@class='v-menu__content theme--light menuable__content__active v-autocomplete__content']");
    ...
    this.dataModelMenuObjectsContainterName = page.getByRole('textbox', { name: 'Name' });
